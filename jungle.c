/*
UFABC - UNIVERSIDADE FEDERAL DO ABC - 2024
Professor - Mario Alexandre Gazziro
Disciplina - Embarcados
Alunos: 
MATHEUS ELOI LOPES RA 11201922121
Vitor Felix do Nascimento RA 11201722021
Adailson Alves De Souza  RA 11102814
...
#define MAX_FLOORS 20		// total # of floors in a stage MUDEI PARA TESTE
...
//define a 4x4 metasprite GORILLA
...
typedef enum FloorItem { ITEM_NONE, ITEM_MINE, ITEM_LEAF, ITEM_POWER };
  ...
  for (floor=0; floor<MAX_FLOORS; floor++) {
	...
	if (lev->gap)
        ...
        if (floor < MAX_FLOORS-1) {
          buf[0]=CH_WALLS+1;		// left side
          buf[COLS-1] = CH_WALLS;	// right side
        }
        // draw ladders
        if (lev->ladder1) {
...
void create_actors_on_floor(byte floor_index) {
  ...
  if (!a->onscreen) {
    ...
    if (floor_index == MAX_FLOORS-1) { //coloca no ultimo andar
      a->name = ACTOR_RESCUE;
      a->state = PACING;
      a->x = 25;//POSICAO DO MACACO RESGATADO
      a->pal = 1; //COR DO MACACO RESGATADO
    }
  }
}
...
void draw_actor(byte i) {
  ...
  switch (a->state) {
    ...
    case CLIMBING:
      ...
      break;
    case PACING:
      meta = metasprite_0_data;//personToSave;
      break;
  }
...
const char* RESCUE_TEXT = 
  "De maos vazias??\n"
  "Eu so queria uma banana\n"
  "e aqui nao tem nenhuma.\n"
  "Vai buscar!!!";
...
void play_scene() {
  ...
  actors[0].pal = 3;// COR DO PERSONAGEM PRINCIPAL
  actors[0].x = 64;
  actors[0].floor = 0;
...
const char PALETTE[32] = { 
  0x0B,			// background color

  0x1A,0x30,0x1B, 0x00,	// ladders and pickups
  0x07,0x17,0x18, 0x00,	// floor blocks
  0x00,0x10,0x20, 0x00,
  0x06,0x16,0x26, 0x00,

  0x16,0x35,0x24, 0x00,	// enemy sprites
  0x0F,0x30,0x07, 0x00,	// rescue person
  0x0D,0x2D,0x3A, 0x00,
  0x17,0x37,0x07	// player sprites
};
...
/*{pal:"nes",layout:"nes"}  CORES DA INTRODUCAO */ 
const unsigned char palTitle[16]={ 0x0F,0x0F,0x0F,0x0F,0x0F,0x19,0x29,0x39,0x0F,0x1A,0x2A,0x3A,0x0F,0x1B,0x2B,0x3B };
void title_screen(void)
{
  //scroll(-8,240);//title is aligned to the color attributes, so shift it a bit to the right

  vram_adr(NAMETABLE_A);
  vram_unrle(telainicio);
...
void main() {
    ...
    title_screen();             // INICIA COM A TELA DE INICIO
